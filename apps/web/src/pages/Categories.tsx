...
export default function CategoriesPage() {
  ...
  const handleTableChange: TableProps<Category>['onChange'] = (paginationInfo, _filters, sorter) => {
...
const columns: ColumnsType<Category> = [
    ...
    { title: '修改时间', dataIndex: 'updatedAt', key: 'updatedAt', sorter: true, sortOrder: sortDir('updatedAt'), width: 110, render: (v) => formatDate(v as string) },
    {
      ...
      render: (_, row) => (
        <Space size={4}>
          <Button type="link" size="small" aria-label={`编辑${row.name}`} onClick={() => openEdit(row)}>编辑</Button>
          <Button type="link" size="small" danger aria-label={`删除${row.name}`} onClick={() => confirmDelete(row)}>删除</Button>
        </Space>
      ),
    },
  ];
