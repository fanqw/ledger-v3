...
const MINIMUM_SAFE_VERSION = [7, 18, 0]
...
test('includes prerelease React Router versions in the routing security check', () => {
  const lockfile = `  react-router@7.18.0-pre.0:\n  react-router-dom@7.18.0:\n`
  const lockedVersions = extractLockedVersions(lockfile)

  assert.deepEqual(lockedVersions, ['7.18.0-pre.0', '7.18.0'])
  assert.equal(isOlderThan(lockedVersions[0], MINIMUM_SAFE_VERSION), true)
})

test('the web app and lockfile use React Router 7.18.0 or newer', async () => {
