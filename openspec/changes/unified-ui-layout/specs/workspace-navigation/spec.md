...
- **WHEN** 用户在不小于 992px 的视口点击收起或展开按钮
...
- **WHEN** 用户在小于 992px 的视口点击顶栏菜单按钮
- **THEN** 系统 SHALL 以抽屉形式显示完整分组导航
- **THEN** 主内容区宽度 SHALL 不因导航打开前的占位而缩小

### Requirement: 响应式布局

系统 SHALL 在桌面端（≥1280px）提供窄侧栏、顶栏和主内容区布局；横屏平板与小桌面（≥992px）保留窄侧栏；竖屏平板与手机（<992px）使用顶栏与抽屉导航，并确保核心操作不被遮挡。
...
- **WHEN** 用户在 992px 至 1279px 宽度的视口访问
...
- **WHEN** 用户登录成功后在不小于 992px 的视口访问任意业务页面
